...
def handle_contact_form():
    ...
    if not request.is_json:
        ...
        return (
            jsonify({"status": "error", "message": "請求格式錯誤，後端需要 JSON"}),
            400,
        )
    ...
    try:
        ...
        if not name or not email or not message:
            ...
            return (
                jsonify(
                    {"status": "error", "message": "請確認姓名、Email 和訊息都已填寫。"}
                ),
                400,
            )
        ...
        return jsonify(
            {"status": "success", "message": "後端：訊息已成功收到，我們會盡快回覆您！"}
        )
        ...
        return (
            jsonify(
                {"status": "error", "message": "後端伺服器處理時發生錯誤，請稍後再試。"}
            ),
            500,
        )
